...
function getTrieAtStep(step, nodeID, isLastLayer) {
  ...
  if (!fs.existsSync(fn)) {
    ...
    if (modelName == "LLAMA") {
      ...
      if (regfault != -1) {
        const command = "REGFAULT=" + regfault.toString() + " mlvm/mlvm" + lastLayer + " --target=" + step.toString() + " --program=" + programPath + " --modelName=" + modelName + " --data=" + inputPath + " --nodeID=" + nodeID.toString() + " --model=" + modelPath + " --prompt=" + prompt
        console.log(command)
